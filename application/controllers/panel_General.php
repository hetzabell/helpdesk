...
class Panel_General extends Controlador_Privado {
    ...
    public function Ver_Ticket($idTicket = '') {
        ...
        //Valida ticket
        $parametros['idIncidencia'] = $idTicket;
        $parametros['idAtiende'] = $this->datos_usuario->idUsuario;
        if (!$this->ticket_model->getTicketValido($parametros)) {
            redirect('404');
        }
        $comentarios['comentarios'] = $this->ticket_model->getVerTicket($idTicket);
        $datosTicket = $comentarios['comentarios'][0];

        //Reactiva el ticket si el usuario asi lo indica
        $this->_continuarAtencion($datosTicket);

        // Actualziar notificaciones como leidas
        $this->_bLecturaNotificaciones($datosTicket);
    ...
    public function Ver_Ticket_Usuario($idTicket = '') {
        ...
        //Valida ticket
        $parametros['idIncidencia'] = $idTicket;
        $parametros['idSolicitante'] = $this->datos_usuario->idUsuario;
        if (!$this->ticket_model->getTickets($parametros)) {
            redirect('404');
        }
    ...
    public function Ver_Ticket_Administrador($idTicket = '') {
        if (!$this->user) {
            redirect('acceso');
        }
        if ($idTicket == '') {
        ...
        //Valida usuario
        if (!($this->datos_usuario->iTipoUsuario == 2)) {
            redirect('404');
        }

        /* Tipo plantilla segun usuario */
        $this->plantilla = 'PanelGeneral/ver_ticket_administrador';
        //=========================================
        //Head de la página
        //=========================================
        //Css exclusivos de esta página
        ...
        $paramInges = array(
            ...
            'idUsuario != ' => $datosTicket->idSolicitante
        );
        $this->vista_contenido['ingenieros'] = $this->usuario_model->getIngenierosActivos($paramInges);
        $this->datos_vista['contenido'] = $this->load->view($this->plantilla, $this->vista_contenido, TRUE);
        //=========================================        
        //footer
        //=========================================        
    ...
    public function llena_tbl_bandeja() {
        ...
        $datosBandeja['idSolicitante !='] = $this->datos_usuario->idUsuario;
        $data['tickets'] = $this->ticket_model->getTickets($datosBandeja);
        $data['idUsuario'] = $this->datos_usuario->iTipoUsuario;
    ...
    public function llena_tbl_atencion() {
        //Esto asegura que sea enviado el tipo de servicio
        if (!$this->user) {
            redirect('acceso');
        }
        ...
        //$datosAbiertos['idAtiende'] = $this->datos_usuario->idUsuario;
        $data['tickets'] = $this->ticket_model->getTickets($datosAbiertos);
        echo $this->load->view('PanelGeneral/parciales/tbl_atencion', $data, TRUE);
    }
    ...
    public function asignar_ticket() {
        ...
        $idIncidencia = $this->input->post('idIncidencia');
        $comentarios = $this->ticket_model->getVerTicket($idIncidencia);
        $datosTicket = $comentarios[0];
        /*         * **Se cierra  crea pausa nueva** */
        $this->_asignacionTercero($datosTicket);
        $idAtiende['idAtiende'] = $this->input->post('idAtiende');
        if ($this->ticket_model->asignaTicket($idIncidencia, $idAtiende)) {
            $status = "Ok";
            $msg = "Ticket asignado con éxito.";
        } else {
            $status = "error";
            $msg = "Error al asignar, intentalo de nuevo.";
        }
        echo json_encode(array('status' => $status, 'msg' => $msg));
    }
    ...
    public function Actualizar_Notificaciones() {
        ...
        } elseif ($this->input->post('iAccion') == 8) {
            $parametros['iStatus < '] = 4;
        } else {
        ...
        $Notificaciones['icon'] = $this->_iconoNotificacion($this->input->post('iAccion'));
        $Notificaciones['modo'] = $this->_enlaceSegunUsuario($this->input->post('iAccion'), $this->datos_usuario->iTipoUsuario);
    ...
    private function _subirAdjuntos($idComentario) {
        if ($_FILES) {
            ...
            foreach ($_FILES as $nombreInput => $archivo) {
                if ($archivo['error'] == 0) {
                    $_FILES[$nombreInput]['name'] = sanear_string($archivo['name']);
    ...
    private function _bLecturaNotificaciones($Ticket) {
        switch ($Ticket->iStatus) {
            case 2:
            case 3:
                $parametros['idIncidencia'] = $Ticket->idIncidencia;

                if ($Ticket->iMotivoPausa == 4 && $Ticket->iStatus == 3)
                    $parametros['iAccion'] = 8;
                else
                    $parametros['iAccion'] = 5;

                $parametros['bLectura'] = 0;
                return $this->ticket_model->updbLecturaNotificacion($parametros);
                break;
            case 4:
                $parametros['idIncidencia'] = $Ticket->idIncidencia;
                $parametros['iAccion'] = 7;
                return $this->ticket_model->updbLecturaNotificacion($parametros);
                break;
            default:
                break;
    ...
    private function _continuarAtencion($Ticket) {
        //Verificar si hay respuesta de usuario
        //para seguir atendiendo
        $parametros['idIncidencia'] = $Ticket->idIncidencia;
        $parametros['iAccion'] = 4;
        if ($this->ticket_model->getNotificacion($parametros)) {
            $this->ticket_model->closePausa($Ticket->idIncidencia, $Ticket->iPausa);
            $this->ticket_model->upStatusIncidencia($Ticket->idIncidencia, 2);
            $parametros['bLectura'] = 0;
            $this->ticket_model->updbLecturaNotificacion($parametros);
        } else {
            $parametros['iAccion'] = 8;
            if ($this->ticket_model->getNotificacion($parametros)) {
                $this->ticket_model->closePausa($Ticket->idIncidencia, $Ticket->iPausa);
    ...
    private function _asignacionTercero($Ticket) {
        switch ($Ticket->iStatus) {
            case 1:
            case 2:
                $datosPausa['idIncidencia'] = $Ticket->idIncidencia;
                $datosPausa['iMotivoPausa'] = 4;
                $this->ticket_model->insPausa($datosPausa);
                $this->ticket_model->upStatusIncidencia($Ticket->idIncidencia, 3);
                $this->ticket_model->insAccionBitacora($Ticket->idIncidencia, $this->datos_usuario->idUsuario, 8);
                break;
            case 3:
                ///Generar cierre de pausa y nueva pausa para la reasignacion
                $this->ticket_model->closePausa($Ticket->idIncidencia, $Ticket->iPausa);
                $datosPausa['idIncidencia'] = $Ticket->idIncidencia;
                $datosPausa['iMotivoPausa'] = 4;
                $this->ticket_model->insPausa($datosPausa);
    ...
    private function _iconoNotificacion($accion) {
        switch ($accion) {
            case 1:
                $icono = 'glyphicon-warning-sign';
                break;
            case 5:
                $icono = 'glyphicon-envelope';
                break;
            case 7:
                $icono = 'glyphicon-ok';
                break;
    ...
    private function _enlaceSegunUsuario($accion, $tipoUsuario) {
        if ($accion == 1 && $tipoUsuario == 2) {
            $enlace = 'Ver-Ticket-Administrador';
        } else {
            $enlace = 'Ver-Ticket';
        }
        return $enlace;

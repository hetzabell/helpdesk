...
class Ticket_model extends CI_Model {
    ...
    function getTicketValido($datos = '') {
        $where = 'idIncidencia = ' . $datos['idIncidencia'] . 
                ' AND (idAtiende = ' . $datos['idAtiende'] .
                ' OR idAtiende = 1)';
        $this->db->where($where);

        $consulta = $this->db->get('vw_Tickets');
        $sql = $this->db->last_query();
        if ($consulta->num_rows() > 0) {
            return TRUE;
        } else {
    ...
    /*
      Update bLectura a uno
     */

    function updbLecturaNotificacion($Parametros) {
        $bLectura['bLectura'] = 1;
        $this->db->where($Parametros);
        return $this->db->update('bitacoraacciones_t', $bLectura);
    }
    ...
    function asignaTicket($idIncidencia, $idUsuario) {
        ...
        if ($query) {
            ...
            $Parametros['iAccion'] = 1;
            $this->updbLecturaNotificacion($Parametros);
